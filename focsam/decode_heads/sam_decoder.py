...
@HEADS.register_module()
class SAMDecoderForRefiner(SAMDecoder):

    def forward(self, inputs, mode='single mask'):
        if mode not in ['single mask', 'single mask with token',
                        'multiple masks with tokens']:
            raise ValueError(f'Unknown mode {mode}, expected '
                             f'"single mask", "single mask with token" or '
                             f'"multiple masks with tokens"')
        masks, mask_tokens_out = self.stem(inputs)
        if mode == 'single mask':
            return masks[:, :1]
        elif mode == 'single mask with token':
            return masks[:, :1], mask_tokens_out[:, :1]
        elif mode == 'multiple masks with tokens':
