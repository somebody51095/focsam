...
class BaseClickSegmentor(BaseInterSegmentor):
    ...
    """
    Implement helper functions for InterSegmentor.
    """
    ...
    def __init__(self,
        ...
        if image_embed_loader is None and backbone is None:
            raise ValueError(f'`image_embed_loader` and `backbone` '
    ...
    def update_clicks(pre_label,
                      ...
                      extra_click_cfg=dict()):  # will be ignored

        if len(extra_click_cfg) > 0:
            warnings.warn(f'`extra_click_cfg` will be ignored')
